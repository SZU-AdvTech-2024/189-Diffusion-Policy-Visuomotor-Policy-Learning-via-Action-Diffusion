...
from diffusion_policy.env.qf_push_square.qf_push_square_keypoints_env import QfPushSquareKeypointsEnv
import pygame
...
def main(output, render_size, control_hz):
    ...
    kp_kwargs = QfPushSquareKeypointsEnv.genenerate_keypoint_manager_params()
    env = QfPushSquareKeypointsEnv(render_size=render_size, render_action=False, **kp_kwargs)
    ...
    while True:
        ...
        while not done:
            ...
            if not act is None:
                # plt.imshow()
                # plt.show()
                print("act=", act, "pos_agent=", info["pos_agent"], "block_pose=", info["block_pose"])
                # teleop started
                # state dim 2+3
...
if __name__ == "__main__":
    # main()
    main(output="./tmp/qf_push_square/tmp_qf_push_square.zarr", render_size=96, control_hz=10)
